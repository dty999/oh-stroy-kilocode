...
version: 1.1.1
description: |
  网文写作工具集基础设施部署。将 hooks/rules/agents/AGENTS.md 等基础设施部署到用户项目目录。
...
你是写作基础设施部署器。将网文写作工具集的全套基础设施（hooks、rules、agents、AGENTS.md）部署到用户项目目录。
...
| `skills/story-setup/references/templates/hooks/` | `.kilo/hooks/` | story-setup managed | recursive replace | `session-*.sh`, `detect-story-gaps.sh`, `validate-story-commit.sh`, `lib/common.sh`, `lib/sentinel.sh` exist |
| `skills/story-setup/references/templates/rules/*.md` | `.kilo/rules/*.md` | story-setup managed | replace | every rule contains `paths` frontmatter |
| `skills/story-setup/references/templates/agents/*.md` | `.kilo/agent/*.md` | story-setup managed | replace | 7 agent files exist |
| `skills/story-setup/references/agent-references/*.md` | `.kilo/skills/story-setup/references/agent-references/*.md` | story-setup managed | replace | every `story-setup/references/agent-references/*.md` reference resolves |
| `skills/story-setup/references/templates/settings-hooks.json` | `.kilo/kilo.json` | user+managed | merge by hook command | hook JSON valid and registered commands deduped |
...
- 部署到项目后，agent 内引用的参考资料必须走 `story-setup/references/agent-references/*.md` 这一本 skill 内复制路径；不要跨 skill 引用其他 skill 的 references。若全局安装路径不同，优先用项目内 `.kilo/skills/` 或 `skills/` 作为规范路径前缀，其次用工具的 skill 搜索能力，不要假定固定绝对路径。

### 2.4.2 部署 Agent References

- 将 `skills/story-setup/references/agent-references/` 下所有 `.md` 复制到项目内 `.kilo/skills/story-setup/references/agent-references/`
- 如目标项目已经使用项目本地 `skills/` 目录，也可以同步复制到 `skills/story-setup/references/agent-references/` 作为 fallback，但不得只复制 fallback 而遗漏 `.kilo/skills/` 主路径
...
- 写入以下字段（YAML `key: value` 格式，hook 用 `references/templates/hooks/lib/sentinel.sh` 读取）：
  ```
  deployed_at: <date -u +"%Y-%m-%dT%H:%M:%SZ">  <!-- Windows: 用 Get-Date -Format "yyyy-MM-ddTHH:mm:ssZ"；Unix: 用 date -u +"%Y-%m-%dT%H:%M:%SZ" -->
  agents_version: 10
  setup_skill_version: 1.1.1
  target_cli: kilo
  resolver_strategy: project-local-skill-reference
  references_dir: .kilo/skills/story-setup/references/agent-references
  ```
...
1. 验证 hooks 注册：
   - 检查 `.kilo/kilo.json` 中的 hooks 字段是否正确
   - 检查 `.kilo/hooks/` 下的脚本是否存在且有执行权限
   - 检查 `.kilo/hooks/lib/common.sh` 与 `.kilo/hooks/lib/sentinel.sh` 是否存在
2. 验证 rules 路径：
   - 检查 `.kilo/rules/` 下的规则文件是否存在且包含 `paths` frontmatter
3. 验证 agents：
   - 检查 `.kilo/agent/` 下的 7 个 agent 定义文件是否存在
4. 验证 agent reference bundle：
   - 检查 `.kilo/skills/story-setup/references/agent-references/` 下 reference 文件完整
...
1. 读取用户现有 `.kilo/kilo.json`（如存在），提取 hooks 部分
...
4. 写入合并后的完整 kilo.json

## 重新部署

- `.story-deployed` 不存在 → 全新安装，Phase 2 全部执行
- `.story-deployed` 存在且 `agents_version: 10` → 提示已部署，AskUserQuestion 确认是否重新部署
- `.story-deployed` 存在但 `agents_version` < 10 → 提示需要更新，重新执行 Phase 2 覆盖 agents/hooks/rules/reference bundle，AGENTS.md 和 kilo.json 走合并策略
...
| references/agent-references/ | Agent 模板自带的参考资料副本；部署到 `.kilo/skills/story-setup/references/agent-references/`，避免跨 skill references |
